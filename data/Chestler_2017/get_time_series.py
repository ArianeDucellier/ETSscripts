...
def get_time_series(index, window, tbegin, tend):
    """
    Function to transform the LFE catalog into a time series

    Input:
        type index = int
        index = Index of the LFE template
        type window = float
...
if __name__ == '__main__':
    ...
    for index in range(0, nt):
        X = get_time_series(index, window, tbegin, tend)
        filename = LFEs[index]['name'][0][0]

"""
This script looks at the catalog from Frank et al. (2014) and plots for
...
for i in range(0, len(families)):
    ...
    plt.xlabel('Time (days) since 2005/01/15', fontsize=24)
    plt.ylabel('Number of LFEs', fontsize=24)
    plt.title('Family {} ({:d} LFEs)'.format(str(families[i]), np.sum(X)), \
        fontsize=24)
    plt.savefig('LFEdistribution/' + str(families[i]) + '.eps', format='eps')
    plt.close(1)

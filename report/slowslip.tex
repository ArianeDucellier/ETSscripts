...
The relatively short recurrence of slow slip and tremor events results in a rich history both in space and time and reveals potential patterns.  These event histories have allowed scientists to see complete event cycles, which is typically not possible to explore in traditional earthquake catalogs, but predicted by numerical models [i.e. Colella et al., 2011, 2012, 2013].

\subsection{Natural hazards}

The mapping of slow slip events along the subduction zone provides insight on the spatial and temporal pattern of strain release.  First order questions that arise from observations of slow slip include how source parameters vary along-strike and what percentage of the plate convergence is released by large slow slip episodes at various locations.  Additionally, the up-dip and down-dip limits of slow slip can be correlated with other geophysical data, such as porosity, temperature, and structure, in an effort to better understand the underlying processes that facilitate aseismic slip.  Slow slip and tremor also constrain the lower edge of the seismogenic zone, a variable that is useful for seismic hazard studies, and event histories are an input into studies that explore the time-dependent loading of stress on the seismogenic zone [Mazzotti and Adams, 2004].

Slow slip and tremor also constrain the lower edge of the seismogenic zone, a variable that is useful for seismic hazard studies, and event histories are an input into studies that explore the time-dependent loading of stress on the seismogenic zone [Mazzotti and Adams, 2004]. At the March, 2013 National Seismic Hazard Maps Workshop in Seattle, it was decided that the up-dip limit of Cascadia tremor would be used as one of the estimates of the down-dip limit of megathrust earthquake rupture. The up-dip limit is determined most uniformly and accurately using our tremor catalog. This will be a key element in future building codes for Cascadia.  

More broadly, we hope to contribute to a better understanding of the mechanism of ETS, and thus improveseismic hazard assessment. Indeed, each episode of slow slip induces a static increase in the stress in the surroundingregion of the megathrust fault, and the possibility of triggering an earthquake. Occurrence of tectonic tremorand LFEs may thus be interpreted as a precursing phenomenon. Moreover, the localization of tectonic tremor andLFEs helps to identify the part of the subducting plate that is slipping, that is the area where an earthquake islikely to occur. Indeed, the region of slow slip is thought to be where fast slip does not occur, that is the up-diplimit of tectonic tremor is the down-dip limit of megathrust earthquake slip. When designing a building code, itis necessary to robustly identify the location of the slipping areas, in order to estimate the possible accelerationsthat a building may undergo. Current building codes are beginning to include data on tremor location, and thusincreasing our knowledge on tremor and LFEs would help designing more reliable building codes.

There exists a temporal and spatial correlation between slow slip, tremor and low frequency earthquakes (LFE). However, the limited resolution of geodetic instruments only allows us to observe the largest slow slip events, and many smaller slow slip events (SSE) may be undetected. Faults are thought to be made of locked unstable patches capable of nucleating earthquakes, embedded in a stable matrix capable of steady or transient aseismic slip. Both tremor/LFEs and slow slip may have the same mechanical origin: tremor may result from small local velocity-weakening asperities that remain locked as the region begins to slip. These patches are stressed by slip on the surrounding fault, ultimately accelerating to high enough slip speed to radiate seismic waves [Bartlow et al., 2011].
Wech and Creager [2011] used tremor location and duration to infer slow slip at the plate interface in Northern Cascadia. Other groups have also developed tremor catalogs for Cascadia using different approaches [Boyarko et al., 2015; Ide, 2012; Sit et al., 2012]. With decreasing depth, we observed a gradation from frequent tremor episodes with small spatial and temporal extent, to infrequent tremor episodes with large spatial and temporal extent [Wech and Creager, 2011]. We hypothesized that the fault weakens with decreasing depth, and that  a continuum of slip size exists in the transition zone between the stable sliding zone and the seismogenic zone. Stable sliding at depth transfers stress to the base of the ETS zone, initiating frequent tremor and small slow slip. In a self-similar process, stress is then transferred updip of the fault, triggering less frequent tremor and larger slow slip, up toward the locked zone.  The same behavior is observed in LFEs with downdip LFE swarms happening nearly weekly [Sweet et al., 2014] and up-dip families only occurring during the yearly ETS events [Sweet, 2014; Chestler and Creager, 2017b]. While it is assumed that a similar behavior exists elsewhere in Cascadia, no one has confirmed this in central or southern Cascadia.

\subsection{Data management}

All scripts to generate figures are on the students GitHub repository, and we will made available to the public.

Where does the data come from (PANGA?)

\subsection{Application to other phenomena}

Monitoring the bridge deformation is the vital task in bridge maintenance and management.
In the first approach, Discrete Wavelet Transform (DWT) is tuned to detect abrupt changes in the response by decomposing the signal into approximation and detail levels. In detail level, decompositions correspond to abrupt changes in the response that might be associated with structural damage.
...
Measurement, 2011, Vol.44(5), pp.927-937 \\
